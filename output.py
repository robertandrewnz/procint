...
Produces:
  ...
  - HTML: output/watchlist_YYYY-MM-DD.html  (dark executive brief)
"""
import json
import logging
...
def _fetch_watchlist() -> list[dict]:
    return db.fetchall(
        ...
        LIMIT  %s
        """,
        (config.WATCHLIST_THRESHOLD, config.TOP_N_WATCHLIST),
    )
...
def _format_bidder_md(b: dict) -> str:
    ...
    parts = [
        f"**{b['firm_name']}** "
        f"({b.get('size', '—')} | strategic: {b.get('strategic_importance', '—')} | "
...
def write_markdown(watchlist: list[dict], output_dir: Path, run_date: date) -> Path:
    ...
    for rank, item in enumerate(watchlist, start=1):
        ...
        bidders_md = (
            ...
            if bidders else "  - No credible bidder matches found"
        )
...
def _sector_badge(sector: str) -> str:
    ...
    return (
        ...
        f'style="background:{colour}22;color:{colour};border-color:{colour}44;">'
        f'{label}</span>'
    )
...
def _bidder_card(b: dict) -> str:
    ...
    context = b.get("company_context")
    if context:
        low_flag = (
            ' <span class="conf-flag" title="Claude has low confidence in this profile">&#9888; low confidence</span>'
            if confidence == "low" else ""
        )
        context_html = f'<div class="bidder-context">{context}{low_flag}</div>'
    elif confidence == "not_run":
        context_html = (
            '<div class="bidder-no-context">'
            'Standard inference &mdash; Claude enrichment not run for this notice'
            '</div>'
        )
    else:
        context_html = ""
    ...
    if bullets:
        bullets_html = "".join(
            f'<div class="bidder-bullet">• {bullet}</div>' for bullet in bullets
        )
        reasoning_html = f'<div class="bidder-reasoning">{bullets_html}</div>'
    else:
        reasoning_html = ""
...
def _recommended_actions(item: dict) -> list[str]:
    ...
    if "EOI" in notice_type or "EXPRESSION" in notice_type:
        actions.append(
            "This is an Expression of Interest — submit a capability statement focused on relevant past "
            "performance rather than pricing. Use it to shape the RFP scope by highlighting differentiators."
        )
    elif "ROI" in notice_type or "REGISTRATION" in notice_type:
        actions.append(
            "This is a Registration of Interest — submit to be included in the shortlist for the formal RFP. "
    ...
    sector_actions = {
        ...
        "defence": (
            ...
            "often require NZ citizenship for key personnel and facility clearances."
        ),
        "health": (
            "Verify Health NZ procurement pathway — some health contracts must be channelled through "
            "national supply agreements or PHARMAC panels."
        ),
        "advisory": (
    ...
    if value_band in ("2m_10m", "10m_plus"):
        actions.append(
            ...
            "Large government contracts often require demonstrated consortium capability."
        )
...
def _notice_card(rank: int, item: dict, bidders: list[dict]) -> str:
    ...
    summary_html = (
        f'<p class="summary-text">{summary}</p>' if summary
        else '<p class="summary-placeholder">AI summary will appear here once enrichment runs.</p>'
    )
    framing_html = (
        f'<div class="framing-block"><span class="framing-label">Strategic framing</span>'
        f'<p>{framing}</p></div>'
        if framing else ""
    )
    flags_html = (
        "".join(f'<div class="flag-item"><span class="flag-icon">&#9888;</span>{f}</div>' for f in flags)
        if flags else '<div class="flag-item no-flags">No red flags identified</div>'
    )
    bidders_html = (
        "".join(_bidder_card(b) for b in bidders)
        if bidders else '<div class="bidder-empty">No credible bidder matches found</div>'
    )
    ...
    return (
        ...
        f'<div class="card-header-main">'
        f'<div class="card-title-row">'
        f'<h2 class="card-title">{title}</h2>'
        f'{_sector_badge(sector)}'
        f'{_dtc_badge(dtc)}'
        f'</div>'
        f'<div class="card-agency">{agency}</div>'
        f'</div>'
        f'<div class="score-block">'
        f'<div class="score-number">{score:.2f}</div>'
        f'<div class="score-label">/ 10</div>'
        f'{_score_bar(score)}'
        f'</div>'
        f'</div>'
        f'<div class="card-meta-row">'
        f'<div class="meta-item"><span class="meta-label">Value</span><span class="meta-value">{value_label}</span></div>'
        f'<div class="meta-item"><span class="meta-label">Close date</span><span class="meta-value">{close_str}</span></div>'
        f'<div class="meta-item"><span class="meta-label">Scope</span><span class="meta-value">{scope}</span></div>'
        f'<div class="meta-item"><span class="meta-label">Notice</span>'
        f'<span class="meta-value"><a href="{url}" target="_blank" rel="noopener">View on GETS &#8599;</a></span></div>'
        f'</div>'
        f'<div class="card-body">'
        f'<div class="col-intel">'
        f'<div class="section-label">Intelligence summary</div>'
        f'{summary_html}'
        f'{framing_html}'
        f'<div class="section-label" style="margin-top:1rem;">Red flags</div>'
        f'<div class="flags-list">{flags_html}</div>'
        f'</div>'
        f'<div class="col-actions">'
        f'<div class="section-label">Recommended actions</div>'
        f'<div class="actions-list">{actions_html}</div>'
        f'</div>'
        f'<div class="col-bidders">'
        f'<div class="section-label">Likely bidders</div>'
        f'<div class="bidders-list">{bidders_html}</div>'
        f'</div>'
        f'</div>'
        f'</div>'
    )
...
<head>
  ...
  <title>Procurement Intelligence &mdash; {run_date}</title>
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    body {{ background:var(--bg); color:var(--text); font-family:'Inter',system-ui,-apple-system,sans-serif; font-size:14px; line-height:1.6; padding:2rem 1.5rem; }}

    /* Header */
    .report-header {{ max-width:1200px; margin:0 auto 2rem; display:flex; align-items:flex-end; justify-content:space-between; border-bottom:1px solid var(--border); padding-bottom:1.25rem; }}
    .report-title {{ font-size:1.1rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:var(--accent); }}
    .report-subtitle {{ font-size:.8rem; color:var(--muted); margin-top:.2rem; }}
    .report-meta {{ text-align:right; font-size:.75rem; color:var(--muted); }}
    .report-meta strong {{ display:block; font-size:1.4rem; font-weight:700; color:var(--text); letter-spacing:-.03em; }}

    /* Explainer */
    .explainer {{ max-width:1200px; margin:0 auto 2rem; background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:1.5rem; display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap:1.5rem; }}
    .explainer-section h3 {{ font-size:.7rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:var(--accent); margin-bottom:.6rem; }}
    .explainer-section p {{ font-size:.78rem; color:var(--muted); line-height:1.6; }}
    .score-breakdown {{ display:flex; flex-direction:column; gap:.4rem; margin-top:.2rem; }}
    ...
    /* Cards */
    .cards {{ max-width:1200px; margin:0 auto; display:flex; flex-direction:column; gap:1.25rem; }}
    .card {{ background:var(--surface); border:1px solid var(--border); border-radius:10px; overflow:hidden; }}
    .card-header {{ display:flex; align-items:flex-start; gap:1rem; padding:1.25rem 1.5rem; border-bottom:1px solid var(--border); background:var(--surf2); }}
    .rank-badge {{ flex-shrink:0; width:2.4rem; height:2.4rem; border-radius:50%; background:var(--border); color:var(--muted); font-size:.75rem; font-weight:700; display:flex; align-items:center; justify-content:center; margin-top:.15rem; }}
    .card-header-main {{ flex:1; min-width:0; }}
    .card-title-row {{ display:flex; align-items:center; flex-wrap:wrap; gap:.5rem; margin-bottom:.25rem; }}
    .card-title {{ font-size:.95rem; font-weight:600; color:var(--text); line-height:1.4; }}
    .card-agency {{ font-size:.78rem; color:var(--muted); }}

    /* Score */
    .score-block {{ flex-shrink:0; text-align:right; min-width:80px; }}
    .score-number {{ font-size:1.75rem; font-weight:800; color:var(--text); letter-spacing:-.04em; line-height:1; }}
    .score-label {{ font-size:.7rem; color:var(--muted); margin-bottom:.4rem; }}
    .score-bar-track {{ height:4px; background:var(--border); border-radius:2px; overflow:hidden; width:80px; }}
    .score-bar-fill {{ height:100%; border-radius:2px; }}

    /* Badges */
    .badge {{ display:inline-flex; align-items:center; padding:.2rem .55rem; border-radius:999px; font-size:.68rem; font-weight:600; letter-spacing:.03em; white-space:nowrap; }}
    ...
    .sector-badge {{ display:inline-flex; align-items:center; padding:.2rem .55rem; border-radius:4px; font-size:.65rem; font-weight:700; letter-spacing:.06em; border:1px solid; }}
    ...
    .meta-label {{ display:block; font-size:.65rem; font-weight:600; letter-spacing:.07em; text-transform:uppercase; color:var(--muted); margin-bottom:.2rem; }}
    ...
    .section-label {{ font-size:.65rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:var(--muted); margin-bottom:.6rem; }}

    /* Summary & framing */
    .summary-text {{ font-size:.83rem; color:var(--text); line-height:1.65; }}
    .summary-placeholder {{ font-size:.8rem; color:var(--muted); font-style:italic; }}
    .framing-block {{ margin-top:1rem; padding:.75rem 1rem; background:#4f9cf908; border-left:2px solid var(--accent); border-radius:0 4px 4px 0; }}
    .framing-label {{ font-size:.65rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:var(--accent); display:block; margin-bottom:.3rem; }}
    .framing-block p {{ font-size:.82rem; color:var(--text); font-style:italic; }}

    /* Flags */
    .flags-list {{ display:flex; flex-direction:column; gap:.4rem; }}
    .flag-item {{ display:flex; align-items:flex-start; gap:.5rem; font-size:.8rem; color:var(--text); }}
    .flag-icon {{ color:#f97316; flex-shrink:0; }}
    ...
    .action-num {{ flex-shrink:0; width:1.3rem; height:1.3rem; border-radius:50%; background:#4f9cf918; border:1px solid #4f9cf940; color:var(--accent); font-size:.65rem; font-weight:700; display:flex; align-items:center; justify-content:center; margin-top:.1rem; }}
    .action-text {{ font-size:.8rem; color:var(--text); line-height:1.6; }}

    /* Bidders — enriched cards */
    .bidders-list {{ display:flex; flex-direction:column; gap:.75rem; }}
    .bidder-card {{ background:var(--surf2); border:1px solid var(--border); border-radius:6px; padding:.65rem .8rem; }}
    .bidder-header {{ display:flex; align-items:center; gap:.5rem; flex-wrap:wrap; margin-bottom:.3rem; }}
    .bidder-name {{ font-size:.82rem; font-weight:600; color:var(--text); flex:1; min-width:0; }}
    .bidder-meta {{ font-size:.72rem; color:var(--muted); }}
    .bidder-pill {{ font-size:.68rem; font-weight:600; padding:.15rem .45rem; border-radius:999px; border:1px solid; white-space:nowrap; }}
    .bidder-context {{ font-size:.78rem; color:var(--muted); line-height:1.55; margin-bottom:.35rem; font-style:italic; }}
    .bidder-reasoning {{ display:flex; flex-direction:column; gap:.2rem; }}
    .bidder-bullet {{ font-size:.74rem; color:#7d8fa8; line-height:1.4; }}
    .conf-flag {{ font-size:.65rem; color:#f97316; margin-left:.3rem; }}
    .bidder-empty {{ font-size:.8rem; color:var(--muted); font-style:italic; }}
    .bidder-no-context {{ font-size:.72rem; color:#4a5568; font-style:italic; margin-bottom:.25rem; border-top:1px solid var(--border); padding-top:.3rem; margin-top:.3rem; }}

    /* Footer */
    .report-footer {{ max-width:1200px; margin:2rem auto 0; padding-top:1rem; border-top:1px solid var(--border); font-size:.72rem; color:var(--muted); display:flex; justify-content:space-between; }}
  </style>
</head>
<body>
  ...
  <div class="report-header">
    <div>
      ...
      <div class="report-subtitle">NZ Government Procurement &mdash; Daily Watchlist</div>
    </div>
  ...
  <div class="explainer">
    <div class="explainer-section">
      <h3>What this is</h3>
      <p>A daily watchlist of active NZ government procurement notices from GETS (gets.govt.nz), scored and ranked for strategic relevance. Notices are ingested each morning, enriched with AI analysis, and matched to likely bidders. Use it to prioritise bid/no-bid decisions and outreach.</p>
    </div>
    <div class="explainer-section">
      <h3>How scores are calculated</h3>
      <p>Each notice is scored 1&ndash;10 across four dimensions, then combined into a composite weighted score.</p>
    ...
    <div class="explainer-section">
      ...
      <p>Sectors are ranked by strategic relevance: <strong style="color:#4f9cf9">FM</strong> and <strong style="color:#f97316">Infrastructure</strong> score highest (0.95/0.90), followed by <strong style="color:#ef4444">Defence</strong>, <strong style="color:#fb923c">Utilities</strong> and <strong style="color:#fb923c">Security</strong> (0.90/0.85), then <strong style="color:#a78bfa">ICT</strong> (0.80), <strong style="color:#34d399">Advisory</strong> (0.70), and other professional services. Weights are tunable in config.py.</p>
    </div>
    <div class="explainer-section">
      <h3>How to use this report</h3>
      <p>Check the <strong>close date badge</strong> first &mdash; red means &le;7 days. Read the <strong>AI summary</strong> and <strong>red flags</strong> to inform go/no-bid. Use <strong>recommended actions</strong> as a starting checklist. <strong>Likely bidders</strong> are keyword-matched and Claude-profiled &mdash; a &#9888; flag means low confidence in the profile.</p>
    </div>
  </div>
...
{cards_html}
  ...
  <div class="report-footer">
    <span>Source: GETS (gets.govt.nz) &middot; Scores and bidder matches computed by Procint Layer 1</span>
